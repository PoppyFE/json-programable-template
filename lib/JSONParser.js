...
class JSONParser {
     ...
     */
    parsePipe(value){
        let _arr = value.split(symbol.recursion);
        return this._recursionPipesSplit(_arr, 0, _arr.length - 1, symbol.defaultOriginValue);
    }
    ...
    _funcSplit(originalValue, arrValue){
        let arr = arrValue.split(symbol.func);
